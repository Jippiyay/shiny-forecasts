...
server <- function(input, output) {
  ...
  refreshPlot <- function(input, output, vals, data_mt) {
    ...
    output$plot1 <- renderPlot({
      ...
      unclicked_vals %>% 
        ...
        scale_x_continuous(name = "Zeit",breaks = scales::breaks_width(10), minor_breaks=seq(from = 0, to = 65, by = 1), labels = scales::number_format(accuracy = 1)) +
        theme_bw() +
  ...
  observeEvent(input$submitbutton, {
    ...
    if(!validatePoints()) {
      ...
      # An diese Stelle muss das speichern eingebaut werden
      resultpoints <- subset(data_mt, clicked %in% c(TRUE))
      colnames(resultpoints) <- c("x", "y", "clicked")
      result <- list(
        date = date(),
        session_id = "whatever",
        points = resultpoints
      ...
      tcres = tryCatch({
        res <- POST(Sys.getenv("APIURL"), body = result, encode = "json", verbose())
        print(res)
