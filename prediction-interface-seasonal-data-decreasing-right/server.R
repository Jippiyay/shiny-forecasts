...
# Create data frame
df_up <- data.frame(time_seasonal = seq(1, 48), seasonal_downward = seasonal_downward)
# Add year labels
...
server <- function(input, output) {
  validatePoints <- function(){
    ...
    linecolors$color <<- "darkgrey"
      if(nrow(clicked_vals) < 6) {
        linecolors[!linecolors$time_seasonal %in% clicked_vals$time_seasonal, "color"] <<- "grey"
          return(FALSE)
      }
    return(TRUE)
  }
  ...
  refreshPlot <- function(input, output, vals, df_up) {
    output$table <- DT::renderDataTable({
      DT::datatable(
        df_up[df_up$clicked == TRUE, c("time_seasonal", "seasonal_downward")],
        editable = "cell",
        ...
        options = list(
          ...
          columnDefs = list(
            list(targets = which(colnames(df_up) == "seasonal_downward"), 
    ...
    output$info <- renderText({
      paste0("Ausgewählter Punkt:\nZeit = ", floor(vals$x), "\nVerkäufe = ", format(round(as.double(df_up$seasonal_downward[df_up$time_seasonal == floor(vals$x)])),big.mark=".", decimal.mark = ","))
      
    })
    
    output$plot1 <- renderPlot({
      print(tail(df_up$seasonal_downward, n= 1))
      ...
      unclicked_vals %>% 
        ggplot(aes(x=time_seasonal, y=seasonal_downward)) +
        geom_line(color="#05095E", data=unclicked_vals) + 
        scale_y_continuous(name = "Verkäufe\n",
                           limits = c(min(df_up$seasonal_downward, na.rm = TRUE)-100, max(df_up$seasonal_downward, na.rm = TRUE)*1.0005), 
        ...
        geom_point(data=df_up[df_up$time_seasonal == floor(vals$x), ], aes(x=time_seasonal, y=seasonal_downward), colour="#000BE6", size =2.3) +
        geom_point(data=clicked_vals[clicked_vals$time_seasonal !=48, ], aes(x=time_seasonal, y=seasonal_downward), colour="#F2A60C")
    })
  }
  ...
  observeEvent(input$plot_click, {
    ...
    if(input$plot_click$x < 1) {
      ...
      if(input$plot_click$x <= endval && input$plot_click$x > offset_tens+1) {
        ...
        print(input)
        if(rlang::is_empty(df_up$seasonal_downward[df_up$time_seasonal == vals$x])) {
          df_up[nrow(df_up) + 1,] <<- c(vals$x, seasonal_downward=input$plot_click$y, year = 5, clicked = TRUE)
        } else {
          df_up$seasonal_downward[df_up$time_seasonal == vals$x] <<- input$plot_click$y
        }
      } else {
  ...
  observeEvent(input$table_cell_edit, {
    ...
    track(event)
    df_up[df_up$time_seasonal == clicked_vals[info$row, "time_seasonal"],"seasonal_downward"] <<- info$value
    print(info)
    refreshPlot(input, output, vals, df_up)
  })
  refreshPlot(input, output, vals, df_up)
